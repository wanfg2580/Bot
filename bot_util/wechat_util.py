...
admin_names = [u'DVN榕树网络-微信小秘书', u'Norman', u'小明同学', u'鸡腿真好吃', u'高瑞鑫']
target_admin_objs = []
target_admin_uns = []
admin_info_dic = {}
...
def group_note(msg):
    ...
    if msg['FromUserName'] in target_group_uns:
        ...
        if invite:
            ...
            return u'欢迎@%s\u2005加入榕树~ 榕树网络' \
                   u'是全球首个由大数据领域资深团队' \
                   u'及行业参与者共建的分布式数据经济生态。' \
                   u'点击直达官网，深度了解榕树：' \
                   u'http://www.banyanbbt.org/ ' % b
...
def do_init():
    ...
    logger.info(u'初始化管理员[%s人]&群[%s个]成功！' % (len(target_admin_uns), len(target_group_uns)))
    logger.info(u'%s' % repr([group['NickName'] for group in target_group_objs]))

    # itchat.update_chatroom(target_group_uns)
    logger.info(u'Update群员信息成功！')


def handle_admin_operation(msg):
    text = msg['Text']
    logger.info(u'开始犹豫管理员消息中是否含有命令...')
    # if msg['Text'].endswith(u'踢'):
    ti_ptn = re.search(r'^@(.*?).bbn$', text)
    if ti_ptn:
        kickee = ti_ptn.group(1)
        logger.info(u'已确认是踢人操作，被踢人：%s.' % kickee)
        ...
        for member in ml:
            if member['NickName'] == kickee:
                ...
                if result['BaseResponse']['Ret'] == 0:
                    return u'\U0001f333%s,亲，系统记录到您违反社区规则，将被暂时移除列入广告灰名单,无法获取榕树最新信息哦~多谢亲们的理解。' % kickee
                    # return u'太可惜了，本群永远失去了%s～' % kickee
    # if text.lstrip().startswith(u'\U0001f333\U0001f333\U0001f333'):
    #     logger.info u'已确认是多群群发，正在发送中...'
    #     for i in target_group_uns:
    #         itchat.send(msg=text, toUserName=i)
    #         time.sleep(random.randint(6, 10))
    #     logger.info u'发送结束'
    #     return u'多群发送结束，小榕给您按摩按摩，辛苦啦么么哒～'
    # logger.info u'好吧，没有命令。。。'
